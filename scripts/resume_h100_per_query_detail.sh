...
echo "Resume mode: preserving existing detail results."
echo "Existing root: ${H100_DETAIL_ROOT}"
...
counts = Counter(int(row["rep"]) for row in rows)
...
if reps != list(range(max(reps) + 1)):
    raise SystemExit(
        f"Non-contiguous repetitions in {path}: {reps}"
    )

for rep in reps:
    if counts[rep] != 200:
        raise SystemExit(
            f"Incomplete repetition in {path}: "
            f"rep={rep}, rows={counts[rep]}"
        )
...
for task in "${TASKS[@]}"; do
    csv_path="${H100_DETAIL_ROOT}/${task}/per_query.csv"
    completed=$(completed_rep_count "${csv_path}")

    echo
    echo "Resume status: task=${task}, completed_reps=${completed}"

    for rep in 0 1 2; do
        if (( rep < completed )); then
            echo "SKIP completed task=${task}, rep=${rep}"
            continue
        fi
...
if total != 1800:
    raise SystemExit(
        f"Expected 1800 rows, got {total}"
    )
PY
...
echo "Expected validator entries: 9 PASS"
echo "Total resumed-task detail rows: 1800"
